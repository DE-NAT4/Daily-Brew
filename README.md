...
Reads the raw CSV file from S3.
Adds column names because the original file has no headers.
Removes sensitive customer information, including customer names and card numbers.
Splits the transaction date and time into separate fields.
Converts text fields to lowercase so the data is more consistent.
Creates unique IDs for transactions and transaction items.
Splits purchased items into separate rows.
Loads the cleaned data into Amazon Redshift.
Uses Grafana dashboards to visualise the data.
AWS Pipeline
